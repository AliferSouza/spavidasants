...
const $state = (() => {
  ...
  return (id, initialValue) => {
    let stateInstance = instancesMap.get(id.id);
    if (!stateInstance) {
      stateInstance = {
        value: initialValue,
        set(newValue) {
          stateInstance.value = newValue;        
          renderComponents(id, id.id);
        },
      };
      instancesMap.set(id.id, stateInstance);
    }
...
function reloadComp(element) {
  ...
  if (useRevalidateValue) {
    ...
    intervalId = setInterval(() => renderComponents(element), parseInt(useRevalidateValue));
  } else {
...
function signal(key, initialValue) {
    const state = globalState[key.id] || { value: initialValue, subscriptions: new Set() };
    globalState[key.id] = state;

    return {
        get value() {
            subscriber && state.subscriptions.add(subscriber);
            return state.value;
        },
        set value(updated) { 
            state.value = updated;
            state.subscriptions.forEach((fn) => fn());        
            if(state.subscriptions.size === 0) renderComponents(event.target, event.target.if);
        },
    };
}
...
function render(fn) {
  subscriber = fn;  
  const result = fn(); // Executa a função de renderização e armazena o resultado
  subscriber = null;
  return result; // Retorna o resultado da função de renderização
}
...
export { Emitter, $state, $effect, $setSearchParam, $useNavigate, $fetch, signal, derived, render};
