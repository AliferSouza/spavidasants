     ...
     <comp-produtos id="promocao"> </comp-produtos> -->
       ...
       <div id="comp-video"></div>
        ...
        `,state:()=>{const t=document.querySelector("#tres-pontos"),s=document.querySelector("#comp-video");let e=!1;t.addEventListener("click",()=>{if(e)s.innerHTML="",e=!1;else{const i=`                    
                       ...
                       </video>`;s.innerHTML=i,e=!0}});const c=$(".SairIa"),r=$("#comp-mascote-ia").tag();c.on("click",()=>{r.style.display=r.style.display==="none"?"block":"none"});function l(){const i=$("#tres-pontos");let u;window.addEventListener("scroll",function(){clearTimeout(u),i.show(),u=setTimeout(function(){i.hide()},4e3)})}l()}}}function I(n){const o=location.hash.split("/").pop().replace("#",""),a=n.Data.dadosC.find(e=>e.id===o);return document.title=o,{html:()=>`
       <comp-menu></comp-menu>
        ...
        `,state:()=>{const e=document.querySelector(".buttonperfil"),c=document.querySelector("#frutas");let r;e.addEventListener("click",l=>{l.preventDefault();const i=c.value;i==="nove"?r="9:00":i==="treze"?r="13:00":i==="quinza"?r="15:00":i==="quartoze"&&(r="16:00");const u=`https://api.whatsapp.com/send?phone=553171685900&text=
           Colaborador:${a.nome}
           Horario:${r} `;window.open(u,"_blank")})}}}async function O(n){document.title="PRODUTO",window.scroll(0,0);const o=location.hash.split("/").pop().replace("#",""),a=n.Data.dadosP.find(e=>e.id===o);return{state:()=>{const e=document.querySelector("#SpanCouter"),c=document.querySelector("#vDoP"),r=document.querySelector("#whats2"),l=parseFloat(c.innerHTML);let i=0,u;$(".infoCotainerButtons").on("click",y=>{if(y.target.className==="infoCotainerButtonsmais"&&(i++,e.innerHTML=i,u=i*l,r.innerHTML=u),y.target.className==="infoCotainerButtonsmenos"&&i>0&&(i--,e.innerHTML=i,u=i*l,r.innerHTML=u),y.target.id==="whats"){const p=`https://api.whatsapp.com/send?phone=553171685900&text= Nome:${a.categoria} Quantidade:${i}, Valor:${u} `;window.open(p,"_blank")}})},html:()=>`
        <comp-menu></comp-menu>
		<div class="cards">   
       ...
       <div class="per-container">
        ...
        `}}async function A(n){const o=n.Data.dadosP.filter(e=>e.categoria===location.hash.split("#").pop()),a=n.Data.dadosP.filter(e=>e.categoria==="oleos")||o;return{state:()=>{const e=JSON.parse(localStorage.getItem("@escolhaUser"))||[];document.querySelectorAll(".perfumaria-container-card").forEach(r=>{const l=e.find(m=>m.id===r.id);let i=0;l&&l.quantidade&&(i=l.quantidade);const u=r.querySelector("#mais"),y=r.querySelector("#menos"),p=r.querySelector("#counter");p.textContent=i,u.addEventListener("click",m=>{i++,p.textContent=i;const f=o.find(g=>g.id===m.target.parentNode.parentNode.id),d=e.find(g=>g.id===f.id);d?(d.valor=parseFloat(d.valor)+parseFloat(f.valor),d.quantidade=(d.quantidade||0)+1):(f.quantidade=1,e.push(f)),localStorage.setItem("@escolhaUser",JSON.stringify(e)),c()}),y.addEventListener("click",m=>{i>0&&(i--,p.textContent=i);const f=o.find(g=>g.id===m.target.parentNode.parentNode.id),d=e.find(g=>g.id===f.id);if(d&&(d.valor=parseFloat(d.valor)-parseFloat(f.valor),d.quantidade=(d.quantidade||0)-1,d.quantidade<=0)){const g=e.indexOf(d);e.splice(g,1)}localStorage.setItem("@escolhaUser",JSON.stringify(e)),c()})});function c(){let r="",l=0,i=0;return e.forEach(m=>{const f=m.nome,d=m.valor,g=m.quantidade;i+=parseFloat(g),l+=parseFloat(d),r+=`Nome: ${f}
Quantidade: ${g}
Valor: R$ ${d}
...
`,document.querySelector("#QTVT").innerHTML=`QT: ${i} VT: ${l}`,`https://wa.me/3171253590?text=${encodeURIComponent(r)}`}c(),document.querySelector("#valor").addEventListener("click",r=>{r.target.getAttribute("key")==="pegarValorEPassarParaWhatsapp"&&window.open(c(),"_blank")})},html:()=>`       
    ...
    `}}const N={home:x,perfil:I,produto:O,personalizado:q,shop:D,perfumaria:A};async function B(n){return n.tag.addEventListener("click",t=>{console.log(t.target)}),{html:()=>`   
  ...
  `,state:()=>{}}}function M(n){return{html:()=>`
    ...
    `,state:()=>{}}}async function _(n){const o=n.Data.dadosP.filter(t=>t.categoria===n.attributes.id);return{html:()=>`
      ...
      <img data-href="/#/produto/#${c.id.split(" ").join("").toLowerCase()}"
           ...
           class="home-img-card"
           img-src="${c.img}"
           alt="${c.nome}" width="200px" height="200px" alt="Produtos">
          `).join("")}
         </div>
        </div>
 ...
 `}}async function z(n){const o=n.Data.dadosP.filter(t=>t.categoria===n.attributes.id);return{html:()=>`
  ...
  `}}function J(){return{html:()=>`
         ...
         <div id="tres-pontos" key="id">                
           ...
           <span></span>
           <span></span>         
        </div>`}}const U={"comp-menu":B,"comp-slider":M,"comp-colaboradores":k,"comp-produtos":_,"comp-newsletter":j,"comp-whatsapp":H,"comp-mascoteia":V,"comp-mascote-ia":R,"comp-perfoleos":z,"comp-button-lateral":J};async function W(){const n=await useCSVToJSON("/data/massagens/massagens.csv");return{dadosC:await useCSVToJSON("/data/colaboradores/colaboradores.csv"),dadosP:n}}document.querySelector("#app");document.querySelector("style");async function F(n={}){const{url:o,method:a,useType:t,cacheDuration:s}=n,e=`${o}_${a}_${t}`,c=localStorage.getItem(e);if(c){const{data:r,timestamp:l}=JSON.parse(c),i=new Date().getTime(),u=s*24*60*60*1e3;if(i-l<u)return r;localStorage.removeItem(e)}try{const r=await fetch(o,a);let l;t==="text"?l=await r.text():t==="json"&&(l=await r.json());const i={data:l,timestamp:new Date().getTime()};return localStorage.setItem(e,JSON.stringify(i)),l}catch{return null}}function Q(n,o,a){if(n==="getItem")return JSON.parse(window.localStorage.getItem(o));if(n==="setItem")window.localStorage.setItem(o,JSON.stringify(a));else if(n==="setItems"){const t=JSON.parse(localStorage.getItem(o)||"[]");t.push(a),localStorage.setItem(o,JSON.stringify(t))}}async function G(){async function n(){return new Promise((o,a)=>{if(navigator.geolocation)navigator.geolocation.getCurrentPosition(t=>{const s=t.coords.latitude,e=t.coords.longitude;o({latitude:s,longitude:e})},t=>{a(t)});else{const t=new Error("Geolocation is not supported by this browser.");a(t)}})}try{const{latitude:o,longitude:a}=await n();return[o,a]}catch(o){return console.log("Erro ao obter localização:",o.message),[null,null]}}async function K(n,o){history.pushState(null,null,n),T(o)}async function X(n){try{const a=new TextEncoder().encode(n),t=await crypto.subtle.digest("SHA-256",a);return Array.from(new Uint8Array(t)).map(c=>c.toString(16).padStart(2,"0")).join("")}catch(o){throw new Error("Erro ao criptografar a senha: "+o)}}async function Y(n,o){const s=(await(await fetch(n,{headers:{ct:o}})).text()).split(`
`),e=s[0].split(";");return s.slice(1).map(c=>c.split(";").reduce((r,l,i)=>{const u={};return u[e[i]]=l,{...r,...u}},{}))}function Z(n,o){return[o.join(","),...n.map(a=>o.map(t=>a[t]||"").join(","))].join(`
`)}const tt=n=>{const o=document.querySelectorAll(n);if(o.length===0)return n;const a={addClass:t=>(o.forEach(s=>{s.classList.add(t)}),a),removeClass:t=>(o.forEach(s=>{s.classList.remove(t)}),a),text:t=>(o.forEach(s=>{s.textContent=t}),a),css:t=>(o.forEach(s=>{Object.assign(s.style,t)}),a),on:(t,s)=>(o.forEach(e=>{e.addEventListener(t,s)}),a),appendImage:t=>(o.forEach(s=>{const e=document.createElement("img");e.src=t,s.appendChild(e)}),a),hide:()=>(o.forEach(t=>{t.style.originalDisplay=t.style.display,t.style.display="none"}),a),show:()=>(o.forEach(t=>{t.style.display=t.style.originalDisplay||"",delete t.style.originalDisplay}),a),tag:()=>o[0],tags:()=>o};return a},T=async(n={},o={})=>{const a=document.querySelector("#app"),[t,s,e]=Object.values(n),c=async()=>{const p=document.getElementsByTagName("img"),m=new IntersectionObserver(function(f,d){f.forEach(function(g){if(g.isIntersecting){const v=g.target,h=v.getAttribute("img-src");v.setAttribute("src",h),v.removeAttribute("img-src"),d.unobserve(v)}})});Array.from(p).forEach(function(f){m.observe(f)})},r=()=>{const p=location.hash.replace("#","")||location.pathname;return p==="/"?Object.keys(t)[0]:p.split("#")[0].split("/").filter(Boolean).pop()};async function l(p={}){let m=[];const f=document.createElement("div");f.insertAdjacentHTML("beforeend",p.renderedHtml);const d=async(h,w)=>{const b=Array.from(h.getAttributeNames()).reduce((E,S)=>S.startsWith("data-")?E:{...E,[S]:h.getAttribute(S)},{}),C={reference:w,nameTag:h.tagName.toLowerCase(),attributes:b,parameter:Object.fromEntries(new URLSearchParams(location.href.split("?")[1]).entries()),page:location.hash.replace("#","").match(/^\/(\w+)(\/)?/),content:h.textContent,Data:e,tag:h},L=h.tagName.toLowerCase();if(s.hasOwnProperty(L)){const{html:E,state:S}=await s[L](C);typeof E=="function"&&(h.innerHTML+=E()),typeof S=="function"&&m.push(S)}else throw new Error(`Componente não encontrado para a tag: ${L}`);await g(h)},g=async h=>{const w=Array.from(h.querySelectorAll("*")).filter(b=>b.tagName.toLowerCase().match(/^comp-/i));for(let b=0;b<w.length;b++){const C=w[b];await d(C)}},v=Array.from(f.querySelectorAll("*")).filter(h=>h.tagName.toLowerCase().match(/^comp-/i));for(let h=0;h<v.length;h++){const w=v[h];await d(w)}a.innerHTML="",a.innerHTML=f.innerHTML,m.forEach(h=>h()),typeof p.renderState=="function"&&p.renderState(),c()}async function i(){const p=r(),m=p&&t[p]?p:"erro";if(m!=="erro"){const f={Pages:t,Components:s,Data:e},{html:d,state:g}=await t[m](f),v=typeof d=="function"?d():await t[m]();l({renderedHtml:v,renderState:typeof g=="function"?g:void 0})}}function u(p,m){let f;return function(...d){clearTimeout(f),f=setTimeout(()=>{p.apply(this,d)},m)}}function y(p){if(p.target.matches("[data-href]")){p.preventDefault();const m=p.target.dataset.href;history.pushState(null,null,m),i()}}window.addEventListener("popstate",i),document.body.addEventListener("click",u(y,200)),i()},P=Object.freeze(Object.defineProperty({__proto__:null,$:tt,Router:T,useApi:F,useCSVToJSON:Y,useId:X,useJSONToCSV:Z,useLocalStorage:Q,useLocation:G,useNavigate:K},Symbol.toStringTag,{value:"Module"}));Object.keys(P).forEach(n=>{typeof P[n]=="function"&&(window[n]=P[n])});async function et(){const n=await W(),o={path:location.origin};Router({pages:N,components:U,data:n},o)}et();
